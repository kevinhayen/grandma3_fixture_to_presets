...
-- The sorting order is defined by variable colorRGBNames, while the RGB value comes from colorRGB
local colorRGBNames		= {'Open', 'Red', 'Orange', 'Amber', 'Yellow', 'Lime', 'Green', 'Cyan', 'Sky Blue', 'Blue', 'Purple', 'UV', 'Magenta', 'Pink', 'Cold White', 'Warm White'}
local colorRGB 			= {};
colorRGB['Open'] 		= '255,255,255'
colorRGB['Red']  		= '255,000,000'
colorRGB['Orange']		= '255,128,000'
colorRGB['Amber']		= '255,191,000'
colorRGB['Yellow']		= '255,255,000'
colorRGB['Lime']		= '128,255,000'
colorRGB['Green']		= '000,255,000'
colorRGB['Cyan']		= '000,255,255'
colorRGB['Sky Blue']	= '033,092,140'
colorRGB['Blue']		= '000,000,255'
colorRGB['Purple']		= '128,000,255'
colorRGB['UV']			= '044,000,255'
colorRGB['Magenta']		= '255,000,255'
colorRGB['Pink']		= '255,000,128'
colorRGB['Cold White']	= '230,255,255'
colorRGB['Warm White']	= '255,255,230'
...
local function fixture_to_presets(displayHandle)
					...
					for channelInfo in ipairs(channelsInfo) do
						...
						if channelsInfo[channelInfo].name == GetVar(UserVars(),"ftpClickedAttribute") or (GetVar(UserVars(),"ftpClickedAttribute") == 'Color' and (channelsInfo[channelInfo].name == 'ColorRGB_R' or channelsInfo[channelInfo].name == 'ColorRGB_C')) then
							...
							if string.sub(channelsInfo[channelInfo].name, 1, 8) == 'ColorRGB' then
								...
								for color in pairs(colorRGBNames) do
									...
									tblOptions[x] = optionsGrid:Append("Button")
									tblOptions[x].Text = colorRGBNames[color]
									tblOptions[x].Anchors = {
										left = y,
										right = y,
										top = x - 1,
	...
	signalTable.SelectionColorRGBAllClick = function(caller)
		...
		for color in pairs(colorRGBNames) do
			fixture_to_presets_storeColorRGB(colorRGBNames[color]);
		end
		SetVar(UserVars(), 'ftpForceMerge', false);
	end
	...
	function fixture_to_presets_storeColorRGB(label)
		...
		dmxValues = fixture_to_presets_split(colorRGB[label], ',')
